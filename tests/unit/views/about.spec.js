...
const spyUnfocus = jest.spyOn(About.methods, 'unfocus')
...
describe('About.vue', () => {
  ...
  describe('focus control', () => {
    test('unfocus tabview nav link', async () => {
      ...
      el.element.focus()
      expect(document.activeElement.className).toEqual('p-tabview-nav-link')

      await el.trigger('click')
      expect(spyUnfocus).toBeCalled()
      expect(document.activeElement.className).not.toEqual('p-tabview-nav-link')
    })
  })
})
